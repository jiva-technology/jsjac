...
return null;}
JSJaCPacket.prototype.getChildVal=function(name,ns){var node=this.getChild(name,ns);if(node&&node.firstChild){return node.firstChild.nodeValue;}else{return'';}};JSJaCPacket.prototype.clone=function(){return JSJaCPacket.wrapNode(this.getNode());};JSJaCPacket.prototype.isError=function(){return(this.getType()=='error');};JSJaCPacket.prototype.getErrorType=function(){return this.getChild('error').firstChild.tagName;};JSJaCPacket.prototype.errorReply=function(stanza_error){var rPacket=this.clone();rPacket.setTo(this.getFrom());rPacket.setFrom();rPacket.setType('error');rPacket.appendNode('error',{code:stanza_error.code,type:stanza_error.type},[[stanza_error.cond]]);return rPacket;};JSJaCPacket.prototype.xml=function(){if(this.getDoc().xml)
...
this.setID(id);return this;};JSJaCIQ.prototype.setQuery=function(xmlns,node){var query;try{query=this.getDoc().createElementNS(xmlns,'query');}catch(e){query=this.getDoc().createElement('query');}
if(query&&query.getAttribute('xmlns')!=xmlns){query.setAttribute('xmlns',xmlns);}
if(node)query.setAttribute('node',node);this.getNode().appendChild(query);return query;};JSJaCIQ.prototype.setCommand=function(xmlns,node){var command;try{command=this.getDoc().createElementNS(xmlns,'command');}catch(e){command=this.getDoc().createElement('command');}
if(command&&command.getAttribute('xmlns')!=xmlns){command.setAttribute('xmlns',xmlns);}
if(node)command.setAttribute('node',node);this.getNode().appendChild(command);return command;};JSJaCIQ.prototype.setCommandX=function(xmlns,type){command=this.getChild('command')
var x;try{x=this.getDoc().createElementNS(xmlns,'x');}catch(e){x=this.getDoc().createElement('x');}
if(x&&x.getAttribute('xmlns')!=xmlns){x.setAttribute('xmlns',xmlns);}
if(type)x.setAttribute('type',type);command.appendChild(x);return x;};JSJaCIQ.prototype.setField=function(target,type,xVar,value){targ=this.getChild(target);field=this.getDoc().createElement('field');field.setAttribute('type',type);field.setAttribute('var',xVar);if(typeof(value)!=='undefined'){for(i=0;i<value.length;i++){v=this.getDoc().createElement('value');valuetext=this.getDoc().createTextNode(value[i]);v.appendChild(valuetext);field.appendChild(v);}}
HTH.log(target);targ.appendChild(field);return targ;}
JSJaCIQ.prototype.getQuery=function(){return this.getNode().getElementsByTagName('query').item(0);};JSJaCIQ.prototype.getSubscriptions=function(){return this.getNode().getElementsByTagName('subscriptions').item(0);};JSJaCIQ.prototype.getQueryXMLNS=function(){if(this.getQuery())
...
JSJaCMessage.prototype=new JSJaCPacket;JSJaCMessage.prototype.setBody=function(body){this._setChildNode("body",body);return this;};JSJaCMessage.prototype.setSubject=function(subject){this._setChildNode("subject",subject);return this;};JSJaCMessage.prototype.setThread=function(thread){this._setChildNode("thread",thread);return this;};JSJaCMessage.prototype.setChatState=function(type){var xmlns='http://jabber.org/protocol/chatstates';var state;try{state=this.getDoc().createElementNS(xmlns,type);}catch(e){state=this.getDoc().createElement(type);}
if(state&&state.getAttribute('xmlns')!=xmlns){state.setAttribute('xmlns',xmlns);}
this.getNode().appendChild(state);return state;};JSJaCMessage.prototype.getThread=function(){return this.getChildVal('thread');};JSJaCMessage.prototype.getBody=function(){return this.getChildVal('body');};JSJaCMessage.prototype.getSubject=function(){return this.getChildVal('subject')};function JSJaCLeaf(){this.base=JSJaCIQ;this.base('leaf');}
JSJaCLeaf.prototype=new JSJaCIQ;JSJaCLeaf.prototype.setPubsub=function(xmlns,node){var query;try{query=this.getDoc().createElementNS(xmlns,'pubsub');}catch(e){query=this.getDoc().createElement('pubsub');}
if(query&&query.getAttribute('xmlns')!=xmlns){query.setAttribute('xmlns',xmlns);}
this.getNode().appendChild(query);return query;}
JSJaCLeaf.prototype.setPublish=function(node){pubsub=this.getChild('pubsub');publish=this.getDoc().createElement('publish');publish.setAttribute('node',node);pubsub.appendChild(publish);return pubsub;}
JSJaCLeaf.prototype.setItem=function(){publish=this.getChild('publish');item=this.getDoc().createElement('item');publish.appendChild(item);return publish;}
JSJaCLeaf.prototype.setEntry=function(xmlns){item=this.getChild('item');entry=this.getDoc().createElementNS(xmlns,'entry')
item.appendChild(entry);return item;}
JSJaCLeaf.prototype.setTitle=function(title){entry=this.getChild('entry');titleelement=this.getDoc().createElement('title');titletext=this.getDoc().createTextNode(title);titleelement.appendChild(titletext);entry.appendChild(titleelement);return this;}
JSJaCLeaf.prototype.setId=function(id){entry=this.getChild('entry');idelement=this.getDoc().createElement('id');idtext=this.getDoc().createTextNode(id);idelement.appendChild(idtext);entry.appendChild(idelement);return this;}
JSJaCLeaf.prototype.setSummary=function(summary){entry=this.getChild('entry');summaryel=this.getDoc().createElement('summary');summarytext=this.getDoc().createTextNode(summary);summaryel.appendChild(summarytext);entry.appendChild(summaryel);return this;}
JSJaCLeaf.prototype.setPublished=function(date){entry=this.getChild('entry');dateNode=this.getDoc().createElement('published');date=this.getDoc().createTextNode(date);dateNode.appendChild(date);entry.appendChild(dateNode);return this;}
JSJaCLeaf.prototype.setSubscriptions=function(){pubsub=this.getChild('pubsub');elm=this.getDoc().createElement('subscriptions');pubsub.appendChild(elm);return pubsub;}
JSJaCLeaf.prototype.setSubscribe=function(node,jid){pubsub=this.getChild('pubsub');publish=this.getDoc().createElement('subscribe');publish.setAttribute('node',node);publish.setAttribute('jid',jid);pubsub.appendChild(publish);return pubsub;}
JSJaCLeaf.prototype.setUnsubscribe=function(node,jid){pubsub=this.getChild('pubsub');publish=this.getDoc().createElement('unsubscribe');publish.setAttribute('node',node);publish.setAttribute('jid',jid);pubsub.appendChild(publish);return pubsub;}
JSJaCLeaf.prototype.setItems=function(node,jid){pubsub=this.getChild('pubsub');publish=this.getDoc().createElement('items');publish.setAttribute('node',node);publish.setAttribute('jid',jid);pubsub.appendChild(publish);return pubsub;}
function JSJaCVcard(){this.base=JSJaCIQ;this.base('vCard');}
JSJaCVcard.prototype=new JSJaCPacket;JSJaCVcard.prototype.setVcard=function(){var query;query=this.getDoc().createElementNS(NS_VCARD,'vCard');query.setAttribute('xmlns',NS_VCARD);query.setAttribute('version','2.0');query.setAttribute('prodid','-//HandGen//NONSGML vGen v1.0//EN');this.getNode().appendChild(query);return query;}
JSJaCVcard.prototype.setName=function(name){vcard=this.getChild('vCard');nameelement=this.getDoc().createElement('FN');nametext=this.getDoc().createTextNode(name);nameelement.appendChild(nametext);vcard.appendChild(nameelement);return this;}
JSJaCVcard.prototype.setDesc=function(desc){vcard=this.getChild('vCard');descelement=this.getDoc().createElement('DESC');desctext=this.getDoc().createTextNode(desc);descelement.appendChild(desctext);vcard.appendChild(descelement);return this;}
JSJaCVcard.prototype.setEmail=function(email){vcard=this.getChild('vCard');emailelement=this.getDoc().createElement('EMAIL');useridelement=this.getDoc().createElement('USERID');emailtext=this.getDoc().createTextNode(email);useridelement.appendChild(emailtext);emailelement.appendChild(useridelement);vcard.appendChild(emailelement);return this;}
JSJaCVcard.prototype.setPhoto=function(photo,type){vcard=this.getChild('vCard');photoelement=this.getDoc().createElement('PHOTO');typeelement=this.getDoc().createElement('TYPE');binvalelement=this.getDoc().createElement('BINVAL');typetext=this.getDoc().createTextNode(type);binvaltext=this.getDoc().createTextNode(photo);typeelement.appendChild(typetext);binvalelement.appendChild(binvaltext);photoelement.appendChild(typeelement);photoelement.appendChild(binvalelement);vcard.appendChild(photoelement);return this;}
JSJaCPacket.wrapNode=function(node){var aNode;switch(node.nodeName.toLowerCase()){case'presence':aNode=new JSJaCPresence();break;case'message':aNode=new JSJaCMessage();break;case'iq':aNode=new JSJaCIQ();break;default:return null;}
...
reqstr+="/>";return reqstr;};JSJaCHttpBindingConnection.prototype._getStreamID=function(slot){this.oDbg.log(this._req[slot].r.responseText,4);if(!this._req[slot].r.responseXML||!this._req[slot].r.responseXML.documentElement){this.oDbg.log('Hello There');this._handleEvent('onerror',JSJaCError('503','cancel','service-unavailable'));return;}
